...
| 命令 | 效果 | 是否改写历史 |
|------|------|--------------|
| `git checkout 哈希` | 临时切到旧版本查看 | 否 |
| `git revert 哈希` | 撤销该次提交（生成反向 commit） | 否，**推荐** |
| `git reset --hard 哈希` | 永久回退到该版本 | 是，**危险** |

```bash
# 临时查看旧版本
git checkout 提交哈希
git checkout main        # 切回最新分支
...
`git log --oneline` 显示的 `2a619ab` 是 **7 位短哈希**，完整版本是 40 位的 SHA-1（如 `2a619ab3c7f1d...`）。

| 显示 | 例子 | 用途 |
|------|------|------|
| 短哈希 | `2a619ab` | 前 7 位，日常使用 |
| 完整哈希 | `2a619ab3c7f1d...` | 40 位（SHA-1）或 64 位（SHA-256） |

- 短哈希足够用：7 位 = 16^7 ≈ 2.6 亿种可能，本地仓库几乎不会撞
- Git 自动给前 7 位
- 平时用短的就行，特殊场景（如 CI 脚本）才用完整

查看完整哈希：

```bash
git rev-parse HEAD       # 当前提交的完整哈希
git log --format=%H      # 显示完整哈希
```
